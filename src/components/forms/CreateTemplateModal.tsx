...
import { useCreateTemplateMutation, useGetCoursesQuery } from '@/store/api';
...
export default function CreateTemplateModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const [formData, setFormData] = useState({
    courseId: '',
    type:     TemplateType.WELCOME as TemplateType,
    name:     '',
    content:  '',
  });
  ...
  const [mediaVideoUrl, setMediaVideoUrl] = useState<string | undefined>();
  const [errors, setErrors]   = useState<Record<string, string>>({});
  const [showVarPanel, setShowVarPanel] = useState(true);
  const courses = coursesData?.data || [];

  // Detect all {{vars}} in content
  ...
  // Append {{var}} to content (cursor-based insert handled inside WhatsAppTextarea)
  ...
  const validate = () => {
    ...
    if (!formData.courseId) e.courseId = 'Please select a course';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: e?.data?.error || 'Failed to create template' });
    }
  };

  const resetForm = () => {
    setFormData({ courseId: '', type: TemplateType.WELCOME, name: '', content: '' });
    setVariableDefaults({});
    setMediaImageUrl(undefined);
    setMediaVideoUrl(undefined);
    setErrors({});
  };

  const handleClose = () => { resetForm(); onClose(); };

  // Live preview: replace auto-fill vars with labels, custom with defaults
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Create Message Template" size="xl">
      <form onSubmit={handleSubmit} className="flex gap-4 h-full">
        ...
        <div className="flex-1 space-y-4 min-w-0">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button type="submit" variant="primary" isLoading={isLoading}>Create Template</Button>
          </div>
        </div>
